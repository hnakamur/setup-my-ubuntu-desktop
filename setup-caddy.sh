...
installed_version=$(caddy --version 2>/dev/null | cut -d ' ' -f 1)
if [ "$installed_version" != v"$version" ]; then
  mkdir -p ~/.local/bin
  arch=$(dpkg --print-architecture)
  curl -fsSL ${repo_url}/releases/download/v${version}/caddy_${version}_linux_${arch}.tar.gz | tar zx -C ~/.local/bin/ caddy 
fi
